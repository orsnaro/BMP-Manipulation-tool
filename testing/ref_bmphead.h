...
struct RGBQUAD;
struct BITMAPDATA;
#pragma pack(1) // allignment : data are put in sets of 1 byte ( no padding at all)
// Pixel Color
struct RGBQUAD
{
	BYTE rgbBlue;
...
#pragma pack(2) //allignment : data are put in sets of 2 bytes if less pad 
// Bitmap file header
struct BITMAPFILEHEADER
{
	union
	...
	WORD    bfReserved1;
	WORD    bfReserved2;
	BITMAPDATA  *bfOffBits =  &BITMAPDATA ;
...
#pragma pack(4) // DWORD allignment 
struct BITMAPDATA
{
